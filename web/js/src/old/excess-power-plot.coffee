...
class Chunk
    ...
    load: (limit=1000, numTries=2) ->
        ...
        url = "#{root}/excesspower/#{ifo}/#{subsystem}/#{channel}/bursts/" + 
...
class BurstStore
    ...
    getWindow: (startTime, endTime, buffer=1) ->
        # Note! Will also return bursts outside of the window!
        starts = @_getChunkStarts startTime, endTime, buffer
        [].concat.apply [], (@chunks[start].bursts for start in starts)
...
class BurstScroller
    ...
    scrollTo: (gpsTime, animateDuration=null) ->
        ...
        timeOffset = (burst, time) -> 
            burst.start_time + burst.start_time_ns*1e-9 - time
        
        {x: mapX, y: mapY} = @scale
        mapZ = @makeZColorMap()
        widthScale = @plotDim.x/@windowSize
        
        bursts = @store.getWindow scrollWindow...
        rects = @canvas.selectAll("rect.value")
                       .data(bursts, (burst) -> burst.id)
        
        describe rects.enter().append("rect"),
            class: "value"
            x: (burst) -> Math.round mapX timeOffset burst, lastTime
            y: (burst) -> 
                Math.round mapY (burst.central_freq - burst.bandwidth/2)
            width: (burst) -> Math.ceil widthScale*burst.duration
            height: (burst) ->
                halfBand = burst.bandwidth/2
                # need to use mapY in case of logarithmic freq scale
                diff = mapY(burst.central_freq - halfBand) - 
                       mapY(burst.central_freq + halfBand)
                Math.ceil Math.abs diff
            stroke: "none"
            fill: (burst) -> mapZ burst.snr
            "clip-path": "url(##{@plotClipId})"
            
        rects.exit().remove()
        
        describe rects.transition().duration(animateDuration).ease("linear"),
             x: (burst) -> Math.round mapX timeOffset burst, gpsTime
...
$ ->
    store = new BurstStore()
    store.start()
    store.indicateWindow startGPSTime, startGPSTime + 30
    
    scroller = new BurstScroller(store, 30, "#plot")
    scroller.setScale
        y: "log"
        z: "log"
